    ...
    <div class="w-full h-300 bg-white shadow-md rounded-none p-8 mt-2 relative">
        <div class="flex justify-between items-center">
            <h2 class="text-2xl font-bold text-gray-800 mb-6">
                {{ $pjbl->nama_syntax }}: {{ $pjbl->mata_pelajaran->nama_mapel }}
            </h2>
        ...
        @if ($pjbl->pengumpulan !== 3)
            <form id="form-pengumpulan" action="{{ route('siswa.kelas.pjbl.pengumpulan') }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    @if ($pjbl->pengumpulan == 3)
        <section class="container mx-auto px lg:px-10 py-12 bg-white rounded-md">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-3xl font-bold text-gray-800">Penugasan</h2>

                @if ($canCreateTugas)
                    <div class="flex space-x-2">
                        <!-- Button Tambah Penugasan -->
                        <button type="button"
            ...
            @else
                <p class="text-gray-600 mt-8">Belum ada penugasan yang ditambahkan.</p>
            @endif

            {{-- Form Tandai Selesai di kanan bawah --}}
            @if ($canCreateTugas)
                <div class="mt-8 flex justify-end">
                    <form action="{{ route('siswa.kelas.pjbl.pengumpulan') }}"
                        method="POST"
