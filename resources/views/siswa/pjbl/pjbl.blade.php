...
<x-layout-siswa>
  ...
  <div class="container mx-auto bg-white p-6 rounded-lg shadow-lg">
    ...
    <div class="flex mt-6" x-data="{ tab: '{{ $pjbls->first()?->slug }}' }">
        <!-- Sidebar Navigasi -->
        @php
    $grouped = $pjbls->groupBy(fn($pjbl) => optional($pjbl->pertemuan)->judul_pertemuan ?? 'Tanpa Pertemuan');
@endphp

    <div class="w-1/4 space-y-4">
        @foreach ($grouped as $judulPertemuan => $pjblGroup)
            <div class="space-y-2">
                <h2 class="text-lg font-semibold text-gray-700">
                    {{ $judulPertemuan }}
                    @php
                        $tanggal = $pjblGroup->first()->pertemuan->tanggal ?? null;
                    @endphp
                    @if ($tanggal)
                ...
                @else
                    ...
                    <div class="mt-4 flex justify-between items-center">
                        ...
                        @if (!isset($pengumpulans[$pjbl->id]))
                            <a href="{{ route('siswa.kelas.pjbl.syntax', ['mapel' => $mapel->slug,'penugasan' => $penugasan->slug, 'pjbl' => $pjbl->slug]) }}"
