<nav class="fixed w-full top-0 z-50 bg-gray-800 text-white p-3 flex justify-between items-center shadow-md">
    ...
    <a href="/guru" class="text-lg font-semibold">Dashboard Guru</a>
    <div class="relative" x-data="{ open: false }">
        <button @click="open = !open" class="flex items-center gap-2 focus:outline-none">
            <span class="hidden md:block text-sm">Pak {{ auth()->user()->nama }}</span>
            <img src="{{ asset('img/' . (Auth::user()->foto ?? 'profile.png')) }}" alt="Admin Profile" class="w-8 h-8 rounded-full border-2 border-white">
        </button>
